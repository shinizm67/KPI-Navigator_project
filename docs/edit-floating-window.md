...
- **`Orbitron`**: Sci-Fi かつ英語（アルファベット表記言語）のみ。
- **`BIZ UDPGothic`**: 日本語・繁體中文（`zh-TW`）など非アルファベット言語、および Office。**数値も含めてすべて BIZ**（混在禁止）。
- 詳細は `docs/local-dev-notes.md` の「フォント（プロダクト共通）」を正とする。
