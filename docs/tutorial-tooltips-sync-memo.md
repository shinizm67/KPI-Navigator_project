...
- ヘッダー: 予約 / DL / 歯車
- Seasonality anomaly の **詳細説明 Tooltip**（Baseline ⚠ / Cockpit 月次配分率合計 ⚠）
- 今後追加する `data-tooltip` 説明文

**非対象（常時可）** — データ閲覧・グラフ値のホバー、および **warning そのもの**

- メモ全文、チャート数値など（別クラスや `title`）
- Seasonality anomaly の **⚠ マーカー / warning color**（Tutorial OFF でも残す。消すのは長い説明だけ）
