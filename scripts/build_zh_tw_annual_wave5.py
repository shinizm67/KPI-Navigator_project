#!/usr/bin/env python3
"""Annual zh-tw Wave 5: Insight overlay + Daily overlay (chrome, section labels, JS hooks)."""
...
def _apply_list(text: str, pairs: list[tuple[str, str]], missing: list[str], label: str) -> str:
    for a, b in sorted(pairs, key=lambda ab: -len(ab[0])):
        if a not in text:
...
def main() -> None:
    ...
    # Annual/Monthly shared polish after bulk replace
    text2 = text2.replace('aria-label="參考 metrics"', 'aria-label="參考指標"')
    text2 = text2.replace('aria-label="支出 metrics"', 'aria-label="支出指標"')
    text2 = text2.replace('aria-label="利潤 metrics"', 'aria-label="利潤指標"')
    text2 = text2.replace(
    ...
    text2 = text2.replace(
        '<span class="insight-daily-reference__label-line">Historical Same Month &amp;</span><br><span class="insight-daily-reference__label-line">Day of Week Average</span>',
        '<span class="insight-daily-reference__label-line">過去同月・</span><br><span class="insight-daily-reference__label-line">同星期平均</span>',
    )
    text2 = text2.replace('daily-overlay__vlabel">Daily</p>', 'daily-overlay__vlabel">每日</p>')
    text2 = text2.replace('daily-overlay__vlabel">Monthly</p>', 'daily-overlay__vlabel">月度</p>')
    text2 = text2.replace('daily-overlay__vlabel">Annual</p>', 'daily-overlay__vlabel">年度</p>')
    ...
    print("build_zh_tw_annual_wave5: OK")
