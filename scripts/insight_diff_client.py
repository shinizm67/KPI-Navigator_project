...
def insight_diff_js() -> str:
    ...
    (function () {{
      ...
      window.renderInsightTwDiffs = function (iso) {{
        ...
        patchAnalyzeMonthlyExpenseCurrent(m, iso);
      }};
      ...
      /* Phase2: Analyze → Monthly Expense 当月1本のみ（MEP daily ÷ mtdA） */
      function patchAnalyzeMonthlyExpenseCurrent(m, iso) {{
        var chart = document.getElementById('insight-analyze-expense-pl-current');
        if (!chart) return;
        if (!m || !iso || typeof window.__insightReadMonthExpense !== 'function') {{
          setExpensePlChartPct(chart, 0, 0);
          return;
        }}
        var parts = String(iso).split('-');
        var y = Number(parts[0]);
        ...
        var exp = window.__insightReadMonthExpense(y, month, day);
        var sales = Number(m.mtdA);
        if (!exp || !exp.hasData || !Number.isFinite(sales) || sales <= 0) {{
          setExpensePlChartPct(chart, 0, 0);
          return;
        }}
        setExpensePlChartPct(
          chart,
          (Number(exp.fixed) / sales) * 100,
          (Number(exp.variable) / sales) * 100
        );
      }}
