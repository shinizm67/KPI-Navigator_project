#!/usr/bin/env python3
"""Insight — Summary Comparison（売上）を実データ化."""
...
MARKER = "patchSummaryComparisonBlocks"
...
def patch_page(path: Path) -> None:
    ...
    if "__sumMonthSalesThroughDay" not in text:
        raise SystemExit(f"__sumMonthSalesThroughDay missing: {path}")
    if "__sumYearSalesThroughDay" not in text:
        raise SystemExit(f"__sumYearSalesThroughDay missing: {path}")
