...
CONFIGS = [
    {
        ...
        "lang": "en",
        "title": "Monthly Edit | KPI Pilot | FORGE LABORATORY",
    ...
    },
    {
        ...
        "lang": "ja",
        "title": "月次編集 | KPI Pilot | FORGE LABORATORY",
        "content_lang_meta": "",
        "translate_no": "",
        "register_css": "../../../register/style.css",
...
{cfg["content_lang_meta"]}  <meta name="viewport" content="width=device-width, initial-scale=1.0">
  ...
  <script>
    /* Basic plan entry guard: block direct navigation to the edit page (pre-render; UX/upsell only, real enforcement is server-side) */
    (function () {{
      try {{
        var k = 'kpiNavigator.subscriptionTier';
        var tier = sessionStorage.getItem(k) || localStorage.getItem(k) || 'pro';
        if (String(tier).trim().toLowerCase() === 'basic') {{
          window.location.replace('{cfg["profit_basic_href"]}');
        }}
      }} catch (_e) {{}}
    }})();
  </script>
